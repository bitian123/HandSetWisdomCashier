...
public class SecurityTool {
    ...
    private static SecurityTool util = new SecurityTool();
...
//		   Log.d("Xor Result:", DataConverter.bytesToHexStringForPrint(xorResult));
    ...
    public static byte lrcCal(byte[] cmdsetb) {
        byte lrc = 0x00;
        for(int  i=0;i<cmdsetb.length;i++){
            lrc ^=  cmdsetb[i];
        }
        return lrc;
    }
...
//            byte[] dataStr = hexStringToByte(ciphertext);
    ...
    public static String generateTempCommKey(String random) {
        ...
        return SecurityTool.getMD5Str(stringBuffer.toString());
    }
...
//		System.out.println("明文：" + HexBinary.encode(data));
    ...
    public static String formatLen(String str){
        int j = 16-str.length()%16;
        if(j==16){
            return str;
        }
    ...
    public static String lrcCal(String str) {
        if(TextUtils.isEmpty(str)){
            return "00";
        }
        byte[] cmdsetb = HexUtil.hexStringToByte(str);
        byte lrc = 0x00;
        for(int  i=0;i<cmdsetb.length;i++){
            lrc ^=  cmdsetb[i];
        }
        int ir = lrc;
        String iStr = Integer.toHexString(ir);
    ...
    public static String getStr16Len(String str){
        if(TextUtils.isEmpty(str)){
            return "0000";
        }
        int len = str.length()/2;
        String len16Str = Integer.toHexString(len);
        if(TextUtils.isEmpty(len16Str)){
            return "0000";
        }else if(len16Str.length()==1){
            return "000"+len16Str;
        }else if(len16Str.length()==2){
            return "00"+len16Str;
        }else if(len16Str.length()==3){
            return "0"+len16Str;
        }
        return len16Str;
    }
