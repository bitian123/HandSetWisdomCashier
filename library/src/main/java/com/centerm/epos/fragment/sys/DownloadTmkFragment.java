 ...
 */
public class DownloadTmkFragment extends BaseFragment {
    private static final String TAG = DownloadTmkFragment.class.getSimpleName();
    ...
    private TextView tvTMKIndex,mTvTip,btn_back;
    ...
    protected void onInitView(View view) {
        ...
        initContentView(view,isManualInputTMK);
    }

    private void initContentView(View view,Boolean isManualInputTMK) {
        tvPwd = (TextView) view.findViewById(R.id.tx_pwd);
        tvPwd.setVisibility(View.VISIBLE);
        etKeyCardPwd = (EditText) view.findViewById(R.id.extxt_key_card_pwd);
        etKeyCardPwd.setVisibility(View.VISIBLE);
        btn_back = (TextView) view.findViewById(R.id.btn_back);
        btn_back.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                ((E10SysMenuActivity)getActivity()).gotoFragment(new DownloadTmkMenuFragment());
            }
        });
        btnTips = (Button) view.findViewById(R.id.btn_tips);
        if (isManualInputTMK) {
            tvPwd.setText(R.string.pls_input_weihu_pwd);
            tvInputTMK = (TextView) view.findViewById(R.id.textView_input_key);
            tvInputTMK.setVisibility(View.VISIBLE);
            tvCheckValue = (TextView) view.findViewById(R.id.textView_input_check);
            tvCheckValue.setVisibility(View.VISIBLE);
            etInputTMK = (EditText) view.findViewById(R.id.extxt_key_encrypted);
            etInputTMK.setVisibility(View.VISIBLE);
            etCheckValue = (EditText) view.findViewById(R.id.extxt_key_check);
            etCheckValue.setVisibility(View.VISIBLE);
            btnTips.setText(R.string.pls_insert_weihu_card);
            initEditWithKeyboard(getActivity(), view.findViewById(R.id.keyboard_view), etInputTMK);
            initEditWithKeyboard(getActivity(), view.findViewById(R.id.keyboard_view), etCheckValue);
            isTransactionKeyCardIn = true;
        } else {
            tvTMKIndex = (TextView) view.findViewById(R.id.textView);
            tvTMKIndex.setVisibility(View.VISIBLE);
            etTMKIndex = (EditText) view.findViewById(R.id.extxt_key_index);
    ...
    public void afterInitView() {
        ...
        try {
            ...
            e.printStackTrace();
            ViewUtils.showToast(getActivity(), "界面初始化异常：" + e.getMessage());
            finishFragment();
        }
    }
    ...
    public void onSureClick(View v) {
        if(TextUtils.equals("确认", btnTips.getText().toString())){
            ...
            if (TextUtils.isEmpty(keyCardPwd) || keyCardPwd.length() != PWD_LEN) {
                ViewUtils.showToast(getActivity(), "请输入"+PWD_LEN+"位密码！");
                return;
            }
    ...
    private void startProcess(){
        ...
        TradeProcess process = config.getTradeProcess(getActivity(), "online2");
        if (process == null) {
        ...
        //启动交易流程
        Intent intent = new Intent(getActivity(), TradeFragmentContainer.class);
    ...
    class ExecuteKeyICCard extends AsyncTask<String, String, String> {
        ...
        protected void onPostExecute(String s) {
            ...
            if (!TextUtils.isEmpty(s)) {
                ViewUtils.showToast(getActivity(), s);
                finishFragment();
            } else {
    ...
    class ExecuteManagerICCard extends AsyncTask<String, String, String> {
        ...
        protected String doInBackground(String... params) {
            ...
            do {
                ...
                try {
                    EncryptAlgorithmEnum encAlg = Settings.getEncryptAlgorithmEnum(getActivity());
    ...
    class CheckICCardInPosition extends AsyncTask<Void, Void, Boolean> {
        ...
        protected void onPostExecute(Boolean b) {
            ...
            if (b) {
                ...
                ViewUtils.showToast(getActivity(), "操作超时！");
                finishFragment();
            }
        }
    }
    ...
    protected int onLayoutId() {
        return R.layout.fragment_ic_tmk_setting;
    }
