...
import com.centerm.epos.task.AsyncDownloadQPSBlackCardBinTask;
...
public class DownloadQpsBlackBinTrade implements ManageTransaction {
    @Override
    public void execute(final ITradeView tradeView, final BaseTradePresent tradePresent) {
        new AsyncDownloadQPSBlackCardBinTask(tradeView.getHostActivity(), tradePresent.getTransData()) {
            ...
            public void onProgress(Integer counts, Integer index) {
                ...
                switch (index) {
                    ...
                    case -2:
                        ...
                        break;
                    default:
                        tradingView.updateHint("正在下载卡BIN C表信息\n起始号" + index);
                        break;
                }
            }
            ...
            public void onFinish(String[] status) {
                ...
                if (TransCode.SIGN_IN.equals(tradePresent.getTradeCode()) && flag != TradeParameterDownController
                    ...
                    TradeParameterDownController controller = TradeParameterDownController.getInstance(tradeView,
                            tradePresent);
                    controller.modityParameterFlag(TradeParameterDownController.QPS_BIN_C_DOWNLOAD_OVER);
                ...
                if ("00".equals(status[0])) {
                    tradePresent.gotoNextStep("2");
                    tradeView.popToast("免密BIN表C黑名单下载成功");
                } else {
