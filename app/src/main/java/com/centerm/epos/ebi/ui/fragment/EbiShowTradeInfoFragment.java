 ...
 * date:2017/3/2</br>
 */
public class EbiShowTradeInfoFragment extends BaseTradeFragment {
...
//            oriTradeInfo.setOriVoucherNum(DataHelper.fillLeftZero(originalMessage.getTraceNumber()+"",6));
//            oriTradeInfo.setOriBatchNo(DataHelper.fillLeftZero(originalMessage.getBatchNumber()+"",6));
    ...
    protected void onInitView(View view) {
        ...
        if (TransCode.VOID.equals(transCode) || TransCode.VOID_SCAN.equals(transCode) || TransCode.COMPLETE_VOID.equals
                (transCode) || TransCode.VOID_INSTALLMENT.equals(transCode)) {
        ...
        addItemView(getString(R.string.label_trans_amt2), DataHelper.formatAmountForShow(oriTradeInfo.getAmount())+"元",
                true);
        if (transCode.equals(TransCode.VOID_SCAN)||transCode.equals(SALE_SCAN_VOID)) {
            addItemView(getString(R.string.label_scan_no), DataHelper.shieldCardNo(oriTradeInfo.getScanCode()), true);
            addItemView(getString(com.centerm.epos.ebi.R.string.label_serial_num_detail_ori_orderid), oriTradeInfo.getScanVoucherNo(), true);
        }else
            addItemView(getString(R.string.label_card_no), DataHelper.shieldCardNo(oriTradeInfo.getCardNo()), true);
        if (TransCode.VOID.equals(transCode) || TransCode.COMPLETE_VOID.equals(transCode) || TransCode
                .VOID_INSTALLMENT.equals(transCode)) {
        ...
        * */
        ...
        if (v.getId() == R.id.positive_btn) {
            ...
            * */
            ...
            } else if (transCode.equals(TransCode.VOID) || transCode.equals(TransCode.VOID_SCAN)
                    || transCode.equals(TransCode.VOID_INSTALLMENT)||transCode.equals(SALE_SCAN_VOID)) {
                dataMap.put(iso_f2, oriTradeInfo.getCardNo());
                dataMap.put(key_oriVoucherNumber, oriTradeInfo.getVoucherNo());
                dataMap.put(JsonKey.mer_order_no, oriTradeInfo.getScanVoucherNo());
                getHostActivity().mTradeInformation.getTransDatas().put(JsonKey.pay_type, oriTradeInfo.getUnicom_scna_type());
                getHostActivity().mTradeInformation.getTransDatas().put(JsonKey.mer_order_no, oriTradeInfo.getScanVoucherNo());
            }
            if (transCode.equals(TransCode.VOID_SCAN)||transCode.equals(SALE_SCAN_VOID)) {
